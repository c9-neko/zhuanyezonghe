    ...
    <div class="main">
      ...
      <div class="content">
        ...
        <div class="button">
          ...
          <el-button class="like" @click="removeCollect" v-if="productDetails.isCollected">取消收藏</el-button>
          <el-button class="like" @click="addCollect" v-else>收藏</el-button>
        </div>
        <!-- 内容区底部按钮END -->
...
export default {
  ...
  computed: {
    ...mapGetters(["getUser"]),
    isLoggedIn() {
      return this.getUser && this.getUser.user_id; // 严格判断用户是否已登录
    }
  },
  methods: {
    ...
    getDetails(val) {
      this.$axios
        .post("/api/product/getDetails", {
          productID: val,
          user_id: this.isLoggedIn ? this.getUser.user_id : null // 只在登录时传递 user_id
        })
        .then((res) => {
